...
public final class CountKeyListener {
    ...
    public static void register() {
        ClientTickEvents.END_CLIENT_TICK.register(client -> {
            ...
            if (down && !wasDown && client.currentScreen == null && client.player != null) {
                ...
                InteractionLog.append(MessageWatcher.capture());
    ...
    private static boolean modifiersHeld(long window, CountKey key) {
        if (key.shift && !eitherKeyDown(window, GLFW.GLFW_KEY_LEFT_SHIFT, GLFW.GLFW_KEY_RIGHT_SHIFT)) {
            return false;
        }
        if (key.ctrl && !eitherKeyDown(window, GLFW.GLFW_KEY_LEFT_CONTROL, GLFW.GLFW_KEY_RIGHT_CONTROL)) {
            return false;
        }
        if (key.alt && !eitherKeyDown(window, GLFW.GLFW_KEY_LEFT_ALT, GLFW.GLFW_KEY_RIGHT_ALT)) {
            return false;
        }
        return true;
    }

    /** True while either of the two keys is down. The keybind capture row uses this too. */
    public static boolean eitherKeyDown(long window, int left, int right) {
